(function () {
  ...
  function showLocationPopup(destinations) {
    ...
    modal.innerHTML = ['<div class="location-box" role="dialog" aria-modal="true" aria-label="Choose your shopping location">','<button type="button" class="location-close" aria-label="Close">&times;</button>','<div class="location-eyebrow">CHOOSE YOUR LOCATION</div>','<h2>Where are you shopping from?</h2>','<p>Select your location to continue with SANTÉ.</p>','<div class="location-options">','<button type="button" class="location-choice philippines">🇵🇭 Philippines<small>Continue to the Philippine SANTÉ product page.</small></button>','<button type="button" class="location-choice global"' + (globalUnavailable ? ' disabled' : '') + '>🌎 Outside the Philippines<small>' + (globalUnavailable ? 'Currently available in the Philippines only.' : 'Continue to the Global SANTÉ product page.') + '</small></button>','</div></div>'].join('');
    document.body.appendChild(modal);
    modal.querySelector('.philippines').addEventListener('click', function () { window.location.assign(destinations.philippines); });
    if (destinations.global) modal.querySelector('.global').addEventListener('click', function () { window.location.assign(destinations.global); });
